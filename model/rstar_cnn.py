...
class RStarCNN(nn.Module):
    """Base class for RStar-CNN.

    This is a base class for RStar CNN links supporting action detection
    ...
    def forward(self, x, gt_bbox, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        sample_roi, secondary_sample_roi, gt_roi_loc, _ = self.proposal_target_creator(
            rois,
            at.tonumpy(gt_bbox),
            [],
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        ...
        # print(f"NUM CLASSES: {self.n_class}")
    ...
    @nograd
    def predict(self, imgs, gt_bboxes, sizes=None,visualize=False):
        ...
        labels = list()
        scores = list()
        for img, gt_bbox, size in zip(prepared_imgs, gt_bboxes.numpy(), sizes):
            img = at.totensor(img[None]).float()
            scale = img.shape[3] / size[1]

            roi_cls_loc, roi_scores, rois, _ = self(img, gt_bbox, scale=scale)
            ...
            # if len(label) == 0:
            #     print(f"DEBUG {prob} {score}")
            # print(f"AFTER: {score} {label}")
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
        ...
        self.use_preset('evaluate')
        self.train()
        # print(f"DEBUG: labels {labels} scores {scores}")
